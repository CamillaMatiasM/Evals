...
async function main() {
  try {
    ...
    const action = await question('Do you want to (r)un an eval or (s)ee an eval? ');
...
async function runEval() {
  try {
    ...
    if (!fs.existsSync(analysisPath)) {
      throw new Error('Analysis folder not found. Please create analysis configurations first.');
    }

    const folders = fs.readdirSync(analysisPath)
      .filter(item => fs.statSync(path.join(analysisPath, item)).isDirectory());
    ...
    const requiredFiles = ['create.json', 'data.jsonl', 'prompt.txt', 'run.json'];
    ...
    if (!evalRecord.eval_ID) {
      ...
      const createConfig = JSON.parse(fs.readFileSync(createConfigPath, 'utf8'));
    ...
    const result = await run(promptFilePath, evalRecord.eval_ID, evalRecord.data_ID, folderPath);
...
async function seeEval() {
  try {
    ...
    evals.forEach((evaluation, index) => {
      const status = evaluation.eval_ID && evaluation.data_ID ? '✅ Ready' : '⏳ Incomplete';
